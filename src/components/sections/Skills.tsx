  ...
  return (
    <section id="skills" aria-labelledby="skills-heading" className="px-6 py-24">
      <div className="max-w-6xl mx-auto">
        <h2 id="skills-heading" className="text-3xl sm:text-4xl font-bold text-center mb-4">Skills</h2>
        ...
        <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.15 }} className="flex flex-wrap justify-center gap-6">
          {skillCategories.map(({ title, icon: Icon, skills }) => (
            <motion.div key={title} variants={item} className="w-full sm:w-[calc(50%-0.75rem)] lg:w-[calc(33.333%-1rem)] rounded-2xl border p-6 flex flex-col gap-4 transition-transform hover:-translate-y-1" style={{ backgroundColor: "var(--bg-elevated)", borderColor: "var(--border)", boxShadow: "var(--card-glow)" }}>
              <div className="w-10 h-10 rounded-full flex items-center justify-center btn-glow">
                <Icon size={18} className="text-white" />
              </div>
              <h3 className="text-lg font-semibold">{title}</h3>
              <div className="flex flex-wrap gap-2">
                {skills.map((skill) => (
                  <span key={skill} className="text-xs px-3 py-1 rounded-full border" style={{ borderColor: "var(--border)", color: "var(--text-secondary)" }}>{skill}</span>
                ))}
              </div>
            </motion.div>
          ))}
