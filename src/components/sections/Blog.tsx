...
export default function Blog() {
  return (
    <section id="learning" aria-labelledby="learning-heading" className="px-6 py-24">
      <div className="max-w-6xl mx-auto">
        <h2 id="learning-heading" className="text-3xl sm:text-4xl font-bold text-center mb-4">Currently Learning</h2>
        ...
        <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.15 }} className="flex flex-wrap justify-center gap-6">
          {topics.map(({ title, description, icon: Icon }) => (
            <motion.div key={title} variants={item} className="w-full sm:w-[calc(50%-0.75rem)] rounded-2xl border p-6 flex gap-4 items-start" style={{ backgroundColor: "var(--bg-elevated)", borderColor: "var(--border)", boxShadow: "var(--card-glow)" }}>
