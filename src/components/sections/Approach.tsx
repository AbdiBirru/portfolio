...
interface Step { title: string; description: string; icon: React.ComponentType<{ size?: number; className?: string }>; }
...
export default function Approach() {
  return (
    <SectionPage id="approach" ariaLabel="Engineering Approach">
      <h2 className="text-3xl sm:text-4xl font-bold text-center mb-4">Engineering Approach</h2>
      ...
      <motion.div variants={container} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.1 }} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {steps.map((step, index) => (
          <motion.div key={step.title} variants={item} className="rounded-2xl border p-6 flex flex-col gap-3" style={{ backgroundColor: "var(--bg-elevated)", borderColor: "var(--border)" }}>
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full flex items-center justify-center btn-glow shrink-0"><step.icon size={18} className="text-white" /></div>
