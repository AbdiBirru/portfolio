...
export default function Hero() {
  ...
  const scrollTo = (id: string) => { document.getElementById(id)?.scrollIntoView({ behavior: shouldReduceMotion ? "auto" : "smooth" }); };

  return (
    <SectionPage id="hero" ariaLabel="Introduction" className="relative overflow-hidden" constrainWidth={false}>
      ...
      <motion.div variants={container} initial="hidden" animate="show" className="relative z-10 max-w-3xl mx-auto text-center flex flex-col items-center gap-6">
        ...
        <motion.p variants={item} className="text-base sm:text-lg max-w-xl" style={{ color: "var(--text-secondary)" }}>I build fast, scalable web applications that turn ideas into real products.</motion.p>
